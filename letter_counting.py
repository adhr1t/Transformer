...
def read_examples(file):
    ...
    return all_lines        # add all the lines of strings to the list all_lines
...
def get_letter_count_output(input: str, count_only_previous: bool=True) -> np.array:
    ...
    """
    output = np.zeros(len(input))   # output is instantiated as a numpy array of zeros that is the same length as the string input
    for i in range(0, len(input)):  # iterate through each character of the input
        if count_only_previous: # if we're only looking at the previous occurrences of the ith character
            output[i] = min(2, len([c for c in input[0:i] if c == input[i]]))   # iterate through all the c characters leading up to the current ith character.
            # If the current cth character is the same as the ith character, then save the cth character in the list. Then we compare the list's length to 2. 
            # If the length is less than 2, then the output[i] is saved as the current length, if it's greater than two, then output[i] is saved as 2
        else:
...
if __name__ == '__main__':
    ...
    # decode(model, dev_bundles[0:5], do_print=True, do_plot_attn=True) # original
